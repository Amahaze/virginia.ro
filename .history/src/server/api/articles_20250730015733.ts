...
router.post('/', upload.fields([
  ...
  try {
    ...
    const title = articleData.title ? JSON.parse(articleData.title) : { ro: '', en: '', ru: '' };
    const content = articleData.content ? JSON.parse(articleData.content) : { ro: '', en: '', ru: '' };
    ...
    const newArticle: NewsArticle = {
      id: uuidv4(),
      title: {
        ro: articleData.title?.ro || '',
        en: articleData.title?.en || '',
        ru: articleData.title?.ru || ''
      },
      content: {
        ro: articleData.content?.ro || '',
        en: articleData.content?.en || '',
        ru: articleData.content?.ru || ''
      },
      imageUrl: articleData.imageUrl,
      imagePosition: articleData.imagePosition as 'left' | 'right' | 'top' | 'bottom',
      link: articleData.link,
      attachmentUrl: articleData.attachmentUrl,
      attachmentName: articleData.attachmentName,
